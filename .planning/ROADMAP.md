...
1. **VERIFY-01**: A user looking at the sidebar sees the Sync-button on rows whose `hasClock=true` and NOT on rows whose `hasClock=false`. Verified visually on `akp03_variant_3004` (post-DEVICES-05 demotion: button hidden), `ak980pro` (`hasClock=true` per ARCH-05.1 — the real `0x28` RTC: button VISIBLE), and any v1.1 catalogue device with `hasClock` still set (button visible).
