...
- [ ] **Phase 10: AKP03 variant_3004 Promotion** — One-line PacketSize 512→1024 fix (unblocks 13 AKP03 sibling SKUs) + real `setKeyImage`/encoder/brightness wired to the 0x3004 LCD + `clock` honest demotion. `scaffolded` → `functional`.
...
### Phase 10: AKP03 variant_3004 Promotion

**Goal**: A user with a `0300:3004` Stream Dock 6-key plugged in can push `QImage`s to any LCD key, see real encoder rotate/press/release events, set per-key colour, set global brightness, clear the device, and flush pending writes — and the `devices.yaml` row no longer falsely advertises `clock` capability. Maturity `scaffolded` → `functional`.
**Depends on**: Phase 9 (ARCH-04 image-pipeline location decided; ARCH-05 `clock` verdict for `akp03_variant_3004` decided; AKP03 0x3004 captures + diff doc committed; `MockTransport` available).
**Requirements**: DISPLAY-01, DISPLAY-02, DISPLAY-03, DISPLAY-04, INPUT-01, INPUT-02, DEVICES-05
**Success Criteria** (what must be TRUE):

1. `src/devices/streamdeck/include/.../akp03_protocol.hpp` PacketSize migrates from 512 → 1024 in one load-bearing commit that simultaneously unblocks the 13 AKP03 sibling SKUs (verified by per-codename + family-coverage tests for both 0x3004 and at least one canonical 0x1001 sibling — Pitfall 30 cross-family regression closed) (DISPLAY-01).
1. A user can push a `QImage` to any LCD key on `akp03_variant_3004` via `IDisplayCapable::setKeyImage(int keyIndex, QImage)`; image preprocessing lives in `src/devices/streamdeck/src/image_pipeline.{hpp,cpp}` per ARCH-04 (60×60 JPEG `Rot0` for 0x3004; AKP03R rev. 2 lineage 64×64 `Rot90` from descriptor table); chunked uploads use 1024-byte packets via `BAT` opcode with last-chunk Transfer-Done flag = 0x01, signature-enforced by `std::span<const uint8_t, 1024>` (Pitfall 18 prevention) (DISPLAY-02).
...
1. A user can cycle through 6 DPI stages on `ajazz_24g_8k` in the firmware-captured cycle order (NOT a naive +1 — Pitfall 28 closed); stage state persists across power-cycle by living in device NVM, NOT host-side cache (verified by power-cycling between cycle commands in a real-hardware test) (MOUSE-02).
1. A user can set per-stage DPI value via `IDpiCapable::setStageDpi(int stage, int dpi)` (cmd 0x21), per-stage colour indicator, and Lift-Off-Distance (cmd 0x23) independently for each of the 6 stages; all wire-format assertions against `MockTransport` (MOUSE-03).
...
**Goal**: A user with an `0c45:8009` AK980 PRO keyboard plugged in (via its 2.4G wireless dongle) can select one of 20 RGB lighting modes, set RGB brightness / speed / direction (raw 0..5 scale shielded by UI scale-mapping), set a discrete sleep-timer, save / push profile changes deliberately (host-save instant; device-flash ≤1/min), and the wireless link does not stall keystrokes during RGB transitions. `devices.yaml` row no longer falsely advertises `clock` capability. Maturity `scaffolded` → `partial`.
**Depends on**: Phase 9 (`ak980pro` Report ID 0x04 / cmd 0x13 / cmd 0x17 captures + diff doc committed; ARCH-05 `clock` verdict for `ak980pro` decided; `isWireless=true` topology evidence captured). Phase 10 (canonical device-promotion template established).
...
1. `docs/_data/devices.yaml` row for `ak980pro` removes `clock` from `capabilities:` with the same ARCH-05 reasoning as DEVICES-05; maturity promotes `scaffolded` → `partial`. RGB + sleep-timer are functional; macros / layers / per-key RGB / battery stay as `feature_summary.pending:` entries per Pitfall 29 honesty contract — deferred to v1.3+ (DEVICES-06).
...
**Depends on**: Phase 9 (`microdia_dongle_7016` topology + HID descriptor captured; ASCII codename verified). Phase 10 (`akp03_variant_3004` is `functional` with `hasClock=false` so Phase 5 Sync-button visibility flips honestly on that row). Phase 12 (`ak980pro` is `partial` with `hasClock=false` so Sync-button visibility flips honestly there too).
...
1. **VERIFY-01**: A user looking at the sidebar sees the Sync-button on rows whose `hasClock=true` and NOT on rows whose `hasClock=false`. Verified visually on `akp03_variant_3004` (post-DEVICES-05 demotion: button hidden), `ak980pro` (post-DEVICES-06 demotion: button hidden), and any v1.1 catalogue device with `hasClock` still set (button visible).
...
| 9. Research, Captures, Hygiene             | v1.2      | 0/?            | Not started      | —          |
