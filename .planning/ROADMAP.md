...
### Phase 29: Plugin GUI Parity — Real-Input Drag-Drop + Property Inspector Config + Multi-Action

**Goal**: A user can fully drive the plugin system from the **real GUI with a real mouse** on Wayland — not just through the debug channel. Closes the user-reported defect "dragging a plugin action onto a button does nothing" and the OpenDeck editing-parity gaps that make the plugin experience feel incomplete: **(1)** a plugin tile dragged with the physical mouse onto a key / dial / touch-zone actually binds and renders on the device; **(2)** an action's **Property Inspector** opens and round-trips settings end-to-end (the PI and the plugin read/write the SAME settings record); **(3)** multiple actions per key + drag-to-reorder/remove, matching OpenDeck's per-button editing. Branch `experiment/mirajazz`. This phase exists because Phase 28 marked drag-to-bind "complete" by verifying the `commitBinding` data path via the debug channel — it **never exercised a real Wayland mouse drag**, which is exactly what is broken.

**Depends on**: Phase 26 (DeviceView/KeyCell/EncoderDial/TouchStripLane drop targets + DragRelay), Phase 28 (affordance payload + commit-path — correct; the gesture layer on top is broken), Phase 20 (PropertyInspectorController + PIBridge + WebEngine — present and compiled), and the experiment/mirajazz plugin-host fixes (Elgato envelope + `plugin_settings_store` + PI relay — landed 2026-06-02, commits 519ecd0..361cc77). UI + app-layer only — no protocol/wire/opcode changes.

**Requirements**: PLUGIN-21 (real-input mouse drag-drop binds on keys/dials/touch-zones, live-verified with driven *pointer* events through the running GUI, not `commitBinding` shortcuts), PLUGIN-22 (Property Inspector usable end-to-end: PI `contextUuid` == the wire context `device#page#controller#row#column` so PI and plugin share one `plugin_settings_store` record; live PI round-trip), PLUGIN-23 (multi-action per key + drag-to-reorder + drag-to-remove editing parity). No parent `.planning/REQUIREMENTS.md` IDs — continuation of the Phase 25/26/27/28 plugin slice.

**Success Criteria**:

1. **Real-input drag-drop binds (PLUGIN-21).** A physical-mouse drag of a plugin tile from the action library onto a `KeyCell` persists the binding and renders the plugin on the device — reproduced and proven via a **driven pointer sequence through the running app** (press on the tile → move over the key → release), NOT a `ProfileController.commitKeyBinding` call. The root cause of the dead drag (the `DragRelay`/`Drag.Internal` gesture path on Wayland) is identified with file:line and fixed. A new debug-channel affordance drives a real drag so this is autonomously regression-tested. Encoder + touch-zone drops verified the same way. ctest `-E qml` >= baseline, 0 failed.
1. **Property Inspector config round-trip (PLUGIN-22).** Selecting a plugin-bound key opens its Property Inspector; the PI's `contextUuid` equals the wire context id (`device#root#Keypad#row#col`) so a setting written in the PI lands in the SAME `plugin_settings_store` record the plugin reads on `willAppear`/`getSettings`. Proven LIVE: set a value in the PI (driven through the running app), confirm the plugin receives it AND the on-disk record matches. The `relayToPropertyInspector` -> `deliverToPropertyInspector` path (361cc77) is exercised end-to-end.
1. **Multi-action + reorder/remove (PLUGIN-23).** A key can hold more than one action (OpenDeck multi-action), actions can be drag-reordered within a key's binding list, and a binding can be removed by drag-to-trash. Persists across restart. Matches OpenDeck's per-button editing model at the QML + ProfileController + core::Profile layer.
1. **Debug-addressability + no regressions.** Every new interactive control sets `objectName`; any new drag/PI driver is exposed on the `AJAZZ_DEBUG_CONTROL` channel. ctest `-E qml` >= current baseline, 0 failed. COD-031 clean. No opcode/`BAT`/`LIG`/hidraw/protocol changes.
...
| 29. Plugin GUI Parity (real drag-drop + PI + multi) | v1.3      | 1/4            | In Progress                     |            |
