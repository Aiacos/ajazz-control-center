...
- [x] E — docs (eee0ae3). CLAUDE.md: new "Stream Dock backend = mirajazz sidecar"
  section ahead of the AKP05E glossary (glossary's C++ refs framed as historical;
  hardware/wire findings still valid); branch ref + latent items refreshed. README:
  architecture diagram/prose show the sidecar-vs-custom split. devices.yaml: backend
  note + akp815 entry fixed (README/wiki AUTOGEN regenerated). docs/architecture/
  PROTOCOLS.md: source-tree + backend description. STATE.md: "Active experiment"
  pointer. Sealed historical artifacts + RE wire-format protocol docs left as-is.
- [x] F — cmake builds + bundles the Rust sidecar cross-platform + CI Rust step
  (284d3ba core, 854013b cross-platform, aa92fc2 analysis-jobs-off). cmake/
  StreamdockSidecar.cmake: ajazz_add_streamdock_sidecar() runs cargo build
  --release --locked, stages beside the app binary + installs (bin/, or
  Contents/MacOS on macOS); macOS universal builds per-arch + lipo. Cargo.lock now
  tracked. CI: rustup stable + cargo cache in ci.yml; rustup (+ both Apple targets
  on macOS) in nightly.yml + release.yml Linux/Windows/macOS jobs;
  AJAZZ_BUILD_SIDECAR=OFF in clang-tidy + CodeQL. Flatpak: app module
  -DAJAZZ_BUILD_SIDECAR=OFF + dedicated streamdock-host module builds offline from
  vendored cargo-sources.json (generated by flatpak-cargo-generator) via the
  rust-stable SDK extension → /app/bin/streamdock-host; packaging/flatpak/README
  documents regeneration. Linux path live-verified (sidecar staged + connects to
  AKP05E); Windows/macOS/Flatpak wired per standard patterns, CI-only.
