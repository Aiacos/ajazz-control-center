...
**Domain:** Per-device capability parity for the 4 physically-connected AJAZZ devices (Stream Dock 6-key, AK980 PRO keyboard, 2.4G 8K mouse, unknown Microdia PID).
...
| ----------------------------------------- | ----------- | ------------------------------------- | ---------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | ---------------- | ---------------------------------- |
| `akp03_variant_3004`                      | `0300:3004` | `usb1/1-13/1-13.3/1-13.3.4`           | `HOTSPOTEKUSB` / `HOTSPOTEKUSB HID DEMO`, iSerial `0300D078140B` | If 0 = vendor HID (EP 0x82 IN 512-B, **EP 0x03 OUT 1024-B**); If 1 = boot keyboard 8-B EP                                                                                            | scaffolded       | `display`, `encoder`, `clock`      |
...
### 1. AKP03 variant 0x3004 — Stream Dock 6-key, scaffolded → functional
...
| Read button + encoder events with action codes from § above    | Without these the device looks dead in our app even though it's connected. The 0x3004 input report semantics are confirmed identical to AKP03 by `opendeck-akp03/src/inputs.rs` (LCD keys 0x01..0x06, side buttons 0x25/0x30/0x31, encoders 0x33..0x35 press + 0x90/0x91/0x50/0x51/0x60/0x61 rotation). | LOW        | `parseInputReport` already in `akp03_protocol.hpp` — the action-code table is correct. Existing AKP03 `partial→functional` plan covers exactly this with the same wire format. Builds on Phase 4 `shared_ptr<IDevice>` lifecycle.                                                                                                                                                        |
...
| ---------------------------------------------------------------- | ---------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Synthesizing a `setSystemTimeOn` wire format for AKP03 family    | "Honor the `clock` capability bit" | The AKP03 protocol DEMONSTRABLY has no RTC opcode (`mirajazz` codepath, opendeck-akp03 codepath, our own `akp03.md` opcode table all confirm). Inventing a packet would be fiction, not clean-room — and there's no host-RTC to set on the device firmware (its time display is renderer-side). | **Demote `clock` capability** from `akp03_variant_3004` in `devices.yaml`. Update `register.cpp` `hasClock=false`. UI Sync button hides cleanly via the existing capability-gating role from Phase 5 (TIMESYNC-04). |
| Treating 0x3004 as fundamentally different from other AKP03 SKUs | "The serial says `HID DEMO`!"      | Descriptor analysis (1024-byte OUT EP, same iManufacturer prefix, same `bNumInterfaces=2`) is conclusive: this is an AKP03-family device with a vendor-internal/early-production iProduct string. Adding a parallel backend doubles maintenance for zero behavioral delta.                      | Route through the existing AKP03 factory (already done in `register.cpp:238-247`), promote tier in lockstep with AKP03 functional sibling.                                                                          |
...
AKP03 v2 framing (1024-B packets)               AK820/AK980 vendor channel (IF 2)
        ...
        │ enables                                      │ enables
        ▼                                              ▼
AKP03 0x3004 image upload                      AK980 PRO RGB modes (0x13)
        │                                              │
        │ enables                                      │ enables
        ▼                                              ▼
AKP03 0x3004 boot-logo + key images           AK980 PRO TFT image upload (0x72)
        │                                              │
...
- [ ] **AKP03 0x3004 promotion to `functional`** — display + encoder both work end-to-end. Requires the v2 framing upgrade above.
...
- [ ] **AKP03 0x3004 `clock` demotion** — `hasClock=false` in `register.cpp` for this variant (since the descriptor proves it's identical wire to AKP03 family which has no RTC opcode).
...
- [ ] **AKP03 0x3004 `SETLB` per-knob RGB** — extends advertised capability to include `rgb` for this device.
...
| AKP03 0x3004 → `functional`                                          | HIGH (user's device)            | LOW (after framing)       | **P1**                                      |
...
| AKP03 0x3004 `clock` demotion                                        | LOW (honesty)                   | LOW                       | **P1**                                      |
...
| AKP03 0x3004 per-knob RGB via `SETLB`                                | LOW-MEDIUM                      | LOW                       | **P2**                                      |
...
- `docs/_data/devices.yaml` lines 264-305 — current `akp03_variant_3004` + `ak980pro` + `ajazz_24g_8k` catalogue entries
