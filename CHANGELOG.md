...
- Property Inspector controller (M1 stub) — `src/app/src/property_inspector_controller.{hpp,cpp}`, exposed to QML as `propertyInspectorController`. Public API surface for hosting plugin-authored HTML PI pages (`loadInspector`, `closeInspector`, `webEngineAvailable`, `hasHtmlInspector`); the M1 milestone is API + build wiring only, every method is currently a no-op so QML bindings work and Qt WebEngine availability can be queried. Subsequent milestones (see TODO.md) implement HTML loading, the QWebChannel bridge, settings persistence, and the WebSocket bridge to the plugin host process. CMake gating is opt-out via `AJAZZ_BUILD_PROPERTY_INSPECTOR` (default ON); when Qt WebEngine isn't found (e.g. minimal Qt installs, headless CI), CMake emits a warning and the build silently degrades to the schema-driven native inspector.
