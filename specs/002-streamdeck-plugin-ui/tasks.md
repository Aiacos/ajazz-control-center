...
- [x] T022 [US3] Verify/wire dial input routing in `src/app/src/plugin_device_bridge.cpp`: rotate→`dialRotate`, press/release→`dialDown`/`dialUp`, tap-on-segment-N→`touchTap` routed to encoder N via the encoder context `device#<pageId>#Encoder#0#N` (the `<pageId>` slot is `ctx.pageId` per `ContextRegistry::deriveContextId`; the default page id is `root`, so the concrete id is `device#root#Encoder#0#N`) (reuse feature-001 paths); confirm segment routing is correct.
  - **Correzione 2026-07-02 (live-verify)**: la mappa bridge era corretta ma su hardware press-only (AKP05E) `dialUp` non partiva MAI — la release sintetica dell'encoder non rientrava nel tap `deviceEvent`, quindi ogni azione plugin `dial_up`-driven era un no-op. Fixed in `stream_dock_input_service.cpp` (release sintetica → deviceEvent; release di filo non più inoltrata al bridge). In più gli eventi OpenDeck `switchProfile`/`deviceBrightness` non erano instradati (kRoutedActions 41→43). Vedi CHANGELOG [Unreleased].
