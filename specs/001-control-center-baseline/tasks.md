...
- [x] T021 [P] [US2] `setTriggerDescription` routing test in `tests/unit/test_plugin_device_bridge.cpp` (RED before T025).
...
- [x] T024 [US2] Inject the modern PI WebSocket bootstrap `connectElgatoStreamDeckSocket(port, context, "registerPropertyInspector", info, actionInfo)` at PI DocumentReady in `src/app/src/property_inspector_controller.cpp` (mirror the HTML-plugin self-bootstrap at `plugin_manager.cpp:606`). **Depends on T023.**

### Implementation for User Story 2 — Outbound contract completion (D3)

- [x] T025 [P] [US2] Wire `setTriggerDescription` into `kRoutedActions` + handler in `src/app/src/sd_plugin_server.cpp` and `src/app/src/plugin_device_bridge.cpp` (manifest field is parsed; runtime command is missing).
- [x] T026 [P] [US2] Render a visual surface for `showAlert`/`showOk` (currently host-ack only) in `src/app/src/plugin_device_bridge.cpp`.
...
- [~] T036 Quickstart walk: **S1 render+brightness PASS** (renderTest paints keys 1-10, brightness set — screenshot read), **S2 keep-alive PASS** (renders after idle, no wedge), **S3 press->plugin->repaint PASS** (sysmon "CPU 15%" — screenshot+log), **S5 encoder/touch PASS** (synthetic `input.encoder`/`encoderPress`/`touch` routed; payload shape headless, raw values HW-gated). **S4 PI round-trip + single didAppear** and **S6 multi-action/toggle** are unit-verified (`[pi-appear]`, `[pi-bridge]`, `[multiaction]`, `[US4]`); their live walks are harness-gated (select/bind synthetic-input limitation; no headless-drivable 2-state/PI-bearing binding).
- [~] T037 Final gate: **`ctest --preset linux-release` GREEN — 845/845** (Linux GCC, -Werror, full build clean). Cross-platform hygiene verified on the diff: no MSVC-fatal APIs (C4996 hook passed on every commit), ASCII-only TEST_CASE names, `#pragma once` headers, Qt/std-only (no platform APIs). The **Apple Clang (-Werror) + MSVC (/W4 /WX)** legs run in CI (`.github/workflows/ci.yml` matrix) on push — cannot be run locally; the shift-left pre-commit gates passed.
