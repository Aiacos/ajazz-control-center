...
| `propertyInspectorDidAppear` / `…DidDisappear` | `{action,context,device,event}` — no payload                                              | **done but DOUBLE-EMIT** (`application.cpp:736` + `:948`) — **dedup required** |
...
| `setTriggerDescription`                              | `{context,rotate?,push?,touch?,longTouch?}`                                         | **MISSING runtime** — manifest field parsed, command not routed |
| `showAlert` / `showOk`                               | `{context,event}`                                                                   | **partial** — host ack only, no visual overlay                  |
