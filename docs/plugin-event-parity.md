...
| `applicationDidLaunch`            | yes             | partial        | `application.cpp` foreground-watcher onChange seam -> `dispatchApplicationLaunchTo`; FOCUS-BASED approximation (see "Focus-based approximation" note), filtered by `ApplicationsToMonitor` | `-R app_lifecycle_events` (registered-only fan-out + ApplicationsToMonitor filter)          |
| `applicationDidTerminate`         | yes             | partial        | `application.cpp` foreground-watcher onChange seam -> `dispatchApplicationTerminateTo`; FOCUS-BASED approximation, filtered by `ApplicationsToMonitor`                                     | `-R app_lifecycle_events` (registered-only fan-out + ApplicationsToMonitor filter)          |
...
- **Focus-based approximation of `applicationDidLaunch` / `applicationDidTerminate`
  (WR-02, status `partial`).** The Elgato/AJAZZ contract defines these as OS
  process launch/terminate events for the apps in a plugin's
  `ApplicationsToMonitor` manifest list. AJAZZ Control Center sources them from the
  **foreground-window watcher** (`IActiveWindowWatcher`), NOT from a process-
  lifecycle monitor. On every foreground CHANGE the host emits
  `applicationDidTerminate(previous foreground)` then `applicationDidLaunch(new foreground)`. Consequences plugin authors must account for:

  - "launch" fires when an already-running app gains focus; "terminate" fires when
    an app merely loses focus (it may still be running). An A->B->A focus bounce
    reports A "launched" twice.
  - The events are NOT tied to real process start/exit. True OS process monitoring
    (logind / Win32 process notifications / `NSWorkspace` launch/terminate) is out
    of scope for this milestone — the watcher is a foreground-window watcher by
    design (34-CONTEXT).
  - **`ApplicationsToMonitor` IS honored for delivery** (WR-02): a plugin receives
    these events only for apps in its `ApplicationsToMonitor` list. An EMPTY (or
    absent) list means "monitor everything" (backwards-compatible delivery for
    every foreground change). The manifest's Elgato object shape
    (`{"mac":[...],"windows":[...]}`) and a bare array are both accepted; tokens
    are normalized (base name, `.exe`/`.app` stripped, lowercased) to the same
    app-identity contract the watcher emits (see ACTIVE-WINDOW-IDENTITY).
    Implemented by `PluginManager::monitorsApplication` + the
    `dispatchApplication{Launch,Terminate}To` filter; delivery remains
    registered-plugin-only (V4, never broadcast).
