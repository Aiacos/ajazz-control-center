...
1. **`resources/icons/keyboard/`**: feature icons for the sidebar
   (Material Symbols are fine; we don't need vendor icons).
1. **No bundled fonts.** Qt's font fallback handles everything.
1. **No bundled GIFs.** `QMovie` handles the user's own GIFs.
