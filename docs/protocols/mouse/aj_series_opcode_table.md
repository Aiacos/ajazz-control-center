...
| `0x83` | `FEA_CMD_GET_BATTERY`            | feature (declared) | **declared but NOT used on the direct mouse path** — battery reads from status report `0x05` byte 3 via GET_FEATURE (§4, hardware-confirmed). The vendor `getBattery()` 0x83 OUTPUT+interrupt path routes via `dangle_dev_type` in `iot_driver` and never replies on a raw hidraw write. | `js:getBattery/commomFeature` |
...
## 4 — Battery model (status report 0x05 byte 3, GET_FEATURE)

> **✅ HARDWARE-CONFIRMED 2026-05-21.** Cross-checked against the
> reverse-engineering corpus on the `research/reverse-engineering` branch
> (`dossier/capture-evidence.md` + `dossier/aj-series-mouse.md`). The mouse
> mirrors its charge into vendor **status report `0x05`, byte 3**, read via
> `GET_FEATURE` on the `0xFFFF`/usage-`0x02` control collection (iface 2,
> `/dev/hidraw9` on Linux). `05 00 00 64 01 01 01 02` (byte 3 = `0x64` = 100%)
> was observed stable on live hardware. This is what
> `AjSeriesMouse::batteryPercent()` implements (commit 1f2be0c) — see the frame
> layout + validation below.
>
> **Two earlier theories were wrong and are kept only as history:**
>
> - **The `0x40` host→device query** — no such opcode on this firmware.
> - **The active `0x83` OUTPUT-write + interrupt-IN query** — *briefly
>   implemented in b9018fc, since reverted.* It never replies on a raw hidraw
>   write, idle OR awake. The reason is now understood: the vendor renderer DOES
>   build `getBattery()` = `r = alloc(8); r[0] = 0x83; commomFeature(r, BIT7)` →
>   `writeFeatureCmd → no() → ao.sendMsg` (OUTPUT) then `readFeatureCmd → oo() → ao.readMsg` (interrupt-IN), parsing percent = `resp[1]`, state = `resp[2]`
>   (1=charge, 2=full), lp = `resp[3]`. **BUT** `sendMsg` is the gRPC
>   `SendMsg{Devicepath, Msg, Checksumtype, Dangledevtype}` — it carries a
>   separate **`dangle_dev_type=MOUSE(2)`** routing argument (computed in the
>   renderer by matching the device against the dongle's `mouseId`), and
>   `iot_driver.exe` folds that routing into the actual HID report bytes (a
>   "get dangle id" handshake precedes it). A direct hidraw `0x83` write carries
>   no such routing, so the dongle ACKs the transfer but never relays the query
>   to the mouse → no interrupt-IN reply. `iot_driver` links the *same hidapi*
>   we do (over `HidD_*`/`WriteFile`/`ReadFile`), so this is **not** a
>   transport/libusb gap — it is missing dongle routing. The directly-readable
>   status report `0x05` byte 3 sidesteps all of it and is the method that
>   works on hardware.
