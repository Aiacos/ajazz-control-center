...
2. **Manifest validation**: rejects if `OS` array does not contain
...
3. **Spawn**:
   ...
   - `.mjs` / `.cjs` / `.js` → `QProcess::start("<install>/node/node20.exe",
     [codePath, "-port", PORT, "-pluginUUID", UUID, "-registerEvent",
     "registerPlugin", "-info", INFO_JSON])`.
   - `.html` → loaded directly in an in-process `QCefView` widget; the
     JS calls `connectElgatoStreamDeckSocket(port, uuid,
     "registerPlugin", info, actionInfo)` to connect back over WS.
4. **WebSocket handshake** (driven by the JS shim, but C++ side mirrors):
...
7. **Restart**: user triggers via "Stop plugin" / restart in the UI.
   `SDPluginManager::restartPlugin(QString uuid)` calls `removePluginCode`
   then `loadPlugin` again.
8. **Shutdown**: at app exit, every plugin process gets a `QProcess::terminate()`
...
2. Host evaluates: if the plugin is in its installed list → respond
   with `{"event": "passHello", "device": "…", "deviceInfo": {…}, "salt": "<random>"}`.
   If not, host immediately closes the connection.
3. (Optional, only when password-protected) Plugin computes
   `secret = sha256(password + salt)` and sends `{"event":
   "authentication", "challenge": "<secret>"}`.
4. Host verifies and either keeps the connection or closes it.
...
1. JS shim wraps to `{event: "setImage", context, payload: {image:
   "data:image/png;base64,…", target: 0}}` and `WebSocket.send()`.
2. Host receives via `SDPluginServer::onTextMessageReceived`.
3. Host parses JSON; resolves `context` to a `(deviceId, page, slot)`
   tuple from its in-memory action map.
4. Host strips the `data:image/png;base64,` prefix, base64-decodes
   ...
   AKP05).
5. Host re-encodes as JPEG (or PNG depending on device — see
   `DataFormatConversion` symbols), via `QImage::save` with format
   `"JPG"` and quality 85.
6. Host calls `SDDevice::appendSendData(ImageStruct&, false)` which
   appends to the per-device `_writeDataList` and wakes
   `_writeDataWaitCondition`.
7. The write thread `SDDevice::writeDataToHidDevice` dequeues, splits
