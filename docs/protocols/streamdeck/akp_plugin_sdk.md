...
| `switchProfile`                                                   | **OpenDeck-only** | `{device, profile}` at the ENVELOPE top level (no `payload`). Sent by the starterpack Switch Profile action (`switch_profile.rs`); host resolves the name and activates the profile (same handler as `switchToProfile`). |
| `deviceBrightness`                                                | **OpenDeck-only** | `{action: set\|increase\|decrease, value}` at the envelope top level (`device_brightness.rs`); host mirrors the SPA brightness slider and the hardware write follows.                                                    |
