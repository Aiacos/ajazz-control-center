...
2. **`SplashScreen.exe`** is spawned via `QProcess::startDetached`
...
3. **`QApplication app(argc, argv)`** — standard Qt construction. Qt
   ...
   `wmfengine.dll`).
4. **Single-instance check**: `QLocalServer::listen("HotSpot_SD_Singleton_<userId>")`.
...
5. **`QSettings` organisation/application name** — set to
   ...
   `758247`).
6. **`LanguageUtilities::initLanguage(QString)`** — loads locale JSON
   from `<install>/translations/`. Languages shipped: `de fr it es pt ja
   zh_CN zh_HK ru ar ko pl` (`streamdock_exe_strings.txt:760673..760685`).
   Default `en.json` is the fallback.
7. **`QtKeychain` init** — instantiated lazily on first secret read; not
   part of bootstrap.
8. **`SDLibrary1.dll`** loaded by delay-binding (its imports appear in
   the EXE import table). On load, `SDLibrary1`'s `DllMain` initialises
   no global state — the `SDDevice` ctor is per-device.
9. **`QCefContext::QCefContext(QCoreApplication*, int, char**, QCefConfig*)`**
...
10. **Main window construction** (`QMainWindow`). At this point the
    splash is still up; the main window is `show()`n after the device
    enumeration kicks off (step 12).
11. **Plugin manager spawn loop** (`SDPluginManager::loadPlugin` /
...
12. **`SDPluginServer::startListen(port)`** — binds the
    ...
    pseudocode).
13. **Device enumeration**: not auto-started by `SDLibrary1`. The exe
...
14. **First device handshake** — see §3.
15. **Splash dismissed**, main window `show()`, `app.exec()` enters the
    Qt event loop.
...
`(int vid, int pid, QString &serial, int interface, int collection,
uchar reportId)` and uses `hid_enumerate` + `hid_open_path` to bind to
...
2. **`startHidWriteThread()`** — spawns `SDGeneralWriteThread` which
   blocks on `_writeDataWaitCondition`.
3. **`sendGetHardwareFirmwareVersion()`** (RVA `0x180023440`) — emits
...
4. **`isOld293Version()`** (RVA `0x18001ed80`) — only for AKP153
...
5. **No clear-display / no reset packet** is emitted at open time. The
...
2. `SetupDiEnumDeviceInterfaces` until the matching path is found.
3. `CreateFileW(devicePath, GENERIC_READ|GENERIC_WRITE, …, OPEN_EXISTING, FILE_FLAG_OVERLAPPED, …)`
4. `WinUsb_Initialize(handle, &winusbInterfaceHandle)`
5. `WinUsb_QueryDeviceInformation` for descriptor metadata.
6. `WinUsb_GetDescriptor(USB_DEVICE_DESCRIPTOR_TYPE)` then walks the
...
7. `sddeviceWinusbInit()` (RVA `0x14b300`) — spawns two
...
8. The first command emitted on WinUSB devices is **also** `CRT VER`
...
2. **Per-device prefs** read on device-open under
   ...
   - `<codename>/<deviceId>/ActionState` — JSON blob of action
     bindings per slot.
3. **SQLite profiles** opened via the embedded SQLite engine
...
4. **Factory-default profiles** loaded from
...
5. **OAuth tokens & passwords** read from QtKeychain on demand
...
2. The protocol carries an **`authentication` challenge-response**:
   ...
   raw WebSocket upgrade) is an `authentication` event with `{event:
   "authentication", challenge: <hash>}`.
...
1. Parse `manifest.json`. Reject if `OS` array does not contain the
   current platform.
2. Resolve `CodePath` / `CodePathWin` / `CodePathMac`. Extension is
   either `.exe` (native), `.mjs` / `.cjs` / `.js` (Node.js), or
   `.html` (CEF in-process).
3. **Native plugins**: spawn directly via `QProcess::start(path, args)`.
   If `RunAsAdministrator: true` in the manifest, the spawn is wrapped
   in a UAC elevation request (Windows only).
4. **Node.js plugins**: spawn `<install>/node/node20.exe` with args
   `<plugin>/plugin/index.js -port <port> -pluginUUID <uuid>
   -registerEvent registerPlugin -info <json>` — these are the exact
   ...
   back to the host.
5. **`.html` plugins**: loaded directly in a `QCefView` inside the
   main process — no subprocess. The JS bridges to the host via
   `QCefQuery` (see [`akp_plugin_sdk.md`](./akp_plugin_sdk.md) §4.3).
6. `QProcess::ProcessError` handler logs `The plugin '%1' crashed: %2`
