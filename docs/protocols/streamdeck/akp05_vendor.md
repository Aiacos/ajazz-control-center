...
> ⚠️ **VENDOR-APP RE vs. LIVE FIRMWARE — read this first.** This document
> describes the **Windows vendor app over WinUSB**. The live `0x0300:0x3004`
> AKP05E over Linux **hidraw** diverges in ways confirmed by driving the panel
> (2026-05-31) — when they disagree, **the hardware findings in
> [`akp05.md`](./akp05.md) win**:
>
> - **All display surfaces use `BAT`.** The strip's 4 zones render via `BAT` at
>   wire bytes 1..4; keys at 6..15. The `DRA`/`MAI`/`ENC` opcodes documented
>   below render **blank** on this firmware (likely WinUSB-only, or need an init
...
| `DRA`                          | 5..7 `D R A`                                               | host→dev  | bytes 8..11 = BE32 size+`0x20`; byte 12 = `location`; bytes 13..14 = BE16 `rect.width`; 15..16 = BE16 `rect.height`; 17..18 = BE16 `rect.x`; 19..20 = BE16 `rect.y`. 32-byte header packet, then JPEG data. | Touch-strip rect-addressable image update (Stream Deck Plus class — partial-update on the strip)                           | `SDDevice::getSecondaryScreenPicInfo`                                                                                                                   | 🟠 **vendor-app only** — `DRA` (and `MAI`) render **blank** on the live `0x0300:0x3004` hidraw firmware (2026-05-31). The strip's 4 zones render via `BAT` at wire bytes 1..4 instead (see [`akp05.md`](./akp05.md)). `DRA` may be WinUSB-only or need an init we don't replicate. |
...
| `ULEND` (5-byte)               | 5..9 `U L E N D`                                           | host→dev  | no payload                                                                                                                                                                                                  | After end-of-image-stream commit                                                                                           | `SDDevice::getUploadFinishedCommand`                                                                                                                    | ✅ **done** — `buildUploadFinished()` sends `ULEND` at buffer offset `5..9` after every image burst; hardware-confirmed working on `0x0300:0x3004` (keys + strip render). mirajazz's offset-3 form also works (device is lenient).                                                 |
...
1. **Wait-for-ACK matrix** in `ImageStruct` confirms the SDK expects the
...
- The reference plugin in `memo.sdPlugin/plugin/index.js` literally calls
  the function `connectElgatoStreamDeckSocket(inPort, inPluginUUID, inRegisterEvent, inInfo)` exactly as Elgato Stream Deck SDK does.
...
1. **Output**: `ghidra_SDLibrary1_dll.json` (1.6 MB) containing:
...
1. **Cross-reference**: full ASCII-strings dump of `SDLibrary1.dll`
   (173 602 strings) and `Stream Dock AJAZZ.exe` (774 312 strings) via a
   PowerShell extractor (`C:\temp\sdlibrary1_strings.txt`,
   `C:\temp\streamdock_exe_strings.txt`).
1. **Clean-room policy**: no vendor source has been incorporated. We used
...
1. **Not yet run** (parking for future passes):
