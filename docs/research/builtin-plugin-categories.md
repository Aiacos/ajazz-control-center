...
| **Casella indicate pagina** | **Page indicator widget**         | Render the current page number on a button face (the "1·2·3" dots visible at the bottom of the screenshot grid is its native form; the per-button widget is a textual counterpart).                                                        | ❌ missing | Pure host-side render once page-cursor exists. **NOT YET TRACKED in TODO.md.**                                                                                                                                                                                                       |
...
1. **System actions (§2) ship in our `ActionEngine`** — no plugin
   ...
   action surface).
1. **Widget categories (§1 rows 3-11) ship as first-party plugin
