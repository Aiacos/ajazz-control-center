...
2. **Mouse interface selection on hidraw.** The two-pass `usage_page`+`usage` →
...
3. **udev access** for VIDs `0x0c45` (keyboard), `0x3151` (mouse), `0x0300`
...
5. **AKP05 wire bytes never confirmed against a live USB capture** — DRA partial-
...
6. **Mouse report-id per opcode** — only config (`0x05`) and clock (`0x00`) are
...
7. **Mouse settings round-trip on hardware** — DPI table `0x54` (8-stage, the
...
8. **Keyboard report-id-0x00 generalisation** — only time-sync + battery are
...
10. **Mouse OLED extras** — OLEDWEATHER `0x2a`, OLEDLANGUAGE `0x27`, OLEDOPTION
    `0x22` byte layouts; SETTFTLCDDATA `0x25` custom-image panel geometry/chunk
    size (54-vs-56); GIF.
11. **AKP815** 100×100/Rot180 device-unconfirmed (probed); 5×3-vs-3×5 orientation;
    dedicated factory.
12. **AKP153 release-edge synthesis** (no byte-10 polarity → frame-diff needed).
13. **Mouse wired-vs-dongle** — does a wired AJ159 accept `0x28`/battery directly,
    or only via the dongle? Only the 2.4G 8K dongle path is verified.
14. **AJ199 dialect split** — OemDrv (17B) vs HIDUsb (20B) vs our 64B; the
...
16. **Anti-features (DO NOT implement):** mouse `getWeather`/`watchSystemInfo`
...
17. **AJ339 / AJ380** — no manifest, no device, never enumerated.
