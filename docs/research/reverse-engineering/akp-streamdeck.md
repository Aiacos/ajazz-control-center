...
2. **AKP815** 100×100/Rot180 transform device-unconfirmed (probed); 5×3 vs 3×5 orientation; dedicated factory.
3. **AKP05 placeholder `0x0300:0x5001`** retirement once a real AKP05 surfaces its true PID.
4. **All AKP05 wire bytes (DRA/ENC/MAI/LOG/M_V) are corpus/Ghidra-derived, never confirmed against a live USB capture.** Top capture targets: a partial-update DRA sequence; the M_V `location==0x12` boot-logo layout; the LOG BE16-vs-BE32 size field; whether N4 accepts `ENC` at all.
5. **report-id-on-Linux** fix is on `feat/linux-device-support`, pending Fedora confirmation — does the AKP05 icon render on hidraw with the `0x00` prepend?
6. **AKP153 release-edge synthesis** (no byte-10 polarity).
