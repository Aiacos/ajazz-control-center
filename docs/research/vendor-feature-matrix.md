...
| Hardware firmware update (over-the-app DFU)            | ✅ confirmed — vendor app ships a separate `FirmwareUpgradeTool.exe` linked against `Qt5SerialPort.dll` (Finding 1+3 in `vendor-protocol-notes.md`, capture id `static-2026-04-26-streamdock-win-001`). USB-CDC bootloader handoff inferred but not yet wire-captured. | ❌ missing                                                                                           | Track once captured. Wire-level capture queued in `vendor-recon-runbook-windows.md` § 2.5.                               |
| Live device-status overlay (e.g. CPU/RAM tile widgets) | ✅ via plugin — vendor ships ELEVEN built-in widget categories enumerated in [`builtin-plugin-categories.md`](builtin-plugin-categories.md) (capture-id `vendor-2026-05-16-streamdock-app-001`).                                                                       | 🟡 partial — Plugin SDK in design (`docs/architecture/PLUGIN-SDK.md`); zero first-party widgets shipped (full per-category gap in [`builtin-plugin-categories.md`](builtin-plugin-categories.md) §1). | "Plugin SDK + Store" in TODO + per-row backlog in [`builtin-plugin-categories.md`](builtin-plugin-categories.md) §4.    |
...
| Polling rate selector                    | ✅ confirmed but vendor marketing exaggerated. AJ159 device manifest exposes `report_max="4"` (4 stages, default 1) — NOT the 5-stage 125/500/1000/4000/8000 ladder marketing claims. Per-device limit set per `report_rate` XML attribute (Finding 9, capture id `static-2026-04-29-aj159-001`).                                                                                                         | ❓ unknown                                               | Wire-capture each stage to recover the byte mapping.          |
| Button remap / macros                    | ✅ confirmed — AJ159 manifest exposes 6 mouse_key entries with rect/`key_value`/`func_type`/`func_value` per button; AJ199 V1.0 `text.xml` exposes `tc_str1` "Times (0-3)" + `tc_str2` "Speed (3-255)" macro repeat semantics (Finding 9).                                                                                                                                                                | ❌ missing                                               | —                                                             |
| Lift-off distance                        | ✅ confirmed — `SUPPORT_LOD_CAL` declared for PixArt 3395/3370/3399/3335/3950 in AJ199 Max `driver_sensor.h` (Finding 10, capture id `static-2026-04-29-aj199max-001`); AJ159 device manifest exposes `show_lod="1"` (Finding 9).                                                                                                                                                                         | ❌ missing                                               | —                                                             |
| Sensor calibration / sleep timeout       | ✅ confirmed — AJ159 manifest exposes `<sleep_light value="30">`, `<move_wakeup>`, `<move_closelight>` per device (Finding 9).                                                                                                                                                                                                                                                                            | ❌ missing                                               | —                                                             |
| Wireless tri-mode (USB-C / 2.4 GHz / BT) | 🟡 partial — AJ159 manifest exposes USB modes (PIDs `5C2E/5D2E/5E2E` on VID `248A`) + 2.4G dongle mode (PID `5C2F` on VID `248A`/`249A`) — that's **dual-mode**, not tri. Bluetooth NOT visible in any AJ-series manifest captured to date. "Tri-mode" marketing on AJ159 / AJ179 may refer to USB-C / 2.4G / dongle-USB rather than including BT. (Finding 8, capture id `static-2026-04-29-aj159-001`). | ❌ missing — we currently expose only the USB transport. | Re-verify against each device's XML during wire-capture pass. |
| Battery level + charging status          | ✅                                                                                                                                                                                                                                                                                                                                                                                                        | ❌ missing                                               | —                                                             |
| Firmware update                          | ✅                                                                                                                                                                                                                                                                                                                                                                                                        | ❌ missing                                               | —                                                             |
