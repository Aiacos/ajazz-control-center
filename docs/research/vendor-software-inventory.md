...
| ---------------------------------- | ------------ | -------------------------------------------------------------------- | ----------- | ------------- | ------------------------------------- | --------------------------------------------------------------------------------------------------------------------- |
| Stream Dock — AJAZZ-branded global | Windows 7+   | unspecified                                                          | 121 620 400 | 2024-01-29    | `a1828628 11703e09 582a009a 6a9a6a90` | `https://hotspot-oss-bucket.oss-cn-shenzhen.aliyuncs.com/custom/AJAZZ/Stream-Dock-AJAZZ-Installer_Windows_global.exe` |
...
| ---------------------------------------- | ------------- | ----------------------- | ----------- | ------------------- | ------------------------------------------------------------------------------------------------------------------------------------------- |
| AK820 Max RGB                            | Windows       | V2.06.01                | 17 983 320  | 2026-04-22          | `https://cdn.shopify.com/s/files/1/0554/6678/6869/files/AJAZZ_AK820_MAX_RGB_Installer_V2.06.01.zip`                                         |
...
| ----------------------------- | ----------- | ------- | ----------- | ----------- | ------------- | --------------------------------------------------------------------------------------------------------------------------- |
| AJ199 (no-RGB, dual-mode)     | PAW3395     | Windows | V1.0        | 2 237 633   | 2026-04-24    | `https://cdn.shopify.com/s/files/1/0554/6678/6869/files/AJAZZ_AJ199_No_RGB_Dual_Modes_PAW3395_Mouse_Driver_V1.0.zip`        |
| AJ199 Max (tri-mode)          | PAW3395     | Windows | unspecified | 7 567 973   | 2026-04-25    | `https://cdn.shopify.com/s/files/1/0554/6678/6869/files/AJAZZ_AJ199_MAX_Triple_Modes_PAW3395_Windows_Mouse_Driver.zip`      |
...
| AJ159 / AJ159 P (dual-mode)   | PAW3395     | Windows | unspecified | 6 020 626   | 2026-04-22    | `https://cdn.shopify.com/s/files/1/0554/6678/6869/files/AJ159_AJ159P_Dual_modes_PAW3395_Win_Mouse_Driver.zip`               |
...
1. **Stream Dock — Mirabox-branded** Windows + macOS installers
   ...
   WebFetch returns 500. Try a real browser session and record the
   raw URLs, sizes, MD5s here — the Mirabox build is a strict
   superset of the AJAZZ build for the AKP-class hardware so its
   protocol log is the more thorough capture target.
1. **Maude keyboard** — confirm whether this is a real product, an
   internal codename, or an alias for an AK-series model. Vendor
   support contact is `support@a-jazz.com`.
1. **AJ339, AJ380** — locate the driver download (or confirm the
   device is firmware-only) before recon planning for these
   sensors.
1. **SHA-256 archival pass** — the current inventory uses
   Content-MD5 as the integrity primitive because that's what the
   Aliyun OSS / shopify CDN return on HEAD. A future pass should
   download every artefact to the encrypted out-of-repo vault and
   record SHA-256 hashes here. SHA-256 is collision-resistant
   against an adversarial CDN; MD5 alone is not.
1. **Version strings**: many "unspecified" entries above are
   shopify URLs that don't encode version in the path. These need
   to be resolved by inspecting the installer's metadata (e.g.
   `7z l file.zip` and reading the embedded build manifest) and
   recorded here. Out of scope for the inventory pass; will be done
   alongside the decompilation step (RE task 2).
