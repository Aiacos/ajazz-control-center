...
> **Status:** alpha. Hot-plug detection live with three device families connected today (Stream Dock, AK980 PRO, 2.4G 8K). Out-of-process sandboxed Python plugin host shipped. In-app Plugin Store with real one-click install mirroring the official AJAZZ Streamdock catalogue (~160 plugins) and the OpenDeck archive (~320 plugins). Cross-platform autostart + native notifications on Linux/macOS/Windows. AK980 PRO 20-mode firmware RGB picker + AJ-series mouse TFT clock+DPI face landed 2026-05-18.
