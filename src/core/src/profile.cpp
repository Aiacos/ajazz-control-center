...
void writeAction(std::ostringstream& out, Action const& a) {
    out << "{";
    out << "\"id\":";       escape(out, a.id);           out << ",";
    out << "\"settings\":"; escape(out, a.settingsJson); out << ",";
    out << "\"label\":";    escape(out, a.label);
    out << "}";
}
...
void writeBinding(std::ostringstream& out, Binding const& b) {
    ...
    out << "{";
    writeChain("onPress", b.onPress);       out << ",";
    writeChain("onRelease", b.onRelease);   out << ",";
...
std::string profileToJson(Profile const& profile) {
    ...
    out << "{";
    out << "\"id\":";       escape(out, profile.id);             out << ",";
    out << "\"name\":";     escape(out, profile.name);           out << ",";
    out << "\"device\":";   escape(out, profile.deviceCodename); out << ",";
