...
TEST_CASE("ak980 setTime start packet: report id 0x00 + data 0x04 + opcode 0x18 (65-byte)",
          "[proprietary][protocol][clock]") {
    // Hardware-verified format (Frida capture 2026-05-21): HID report id 0x00 at
    // byte 0, the 0x04 'ReportId' constant is the first DATA byte at byte 1.
    auto const pkt = buildSetTimeStart();
    REQUIRE(pkt.size() == TimeReportSize);
    REQUIRE(pkt[0] == 0x00);         // HID report id (unnumbered)
    REQUIRE(pkt[1] == ReportId);     // 0x04 - first data byte
    REQUIRE(pkt[2] == CmdStartTime); // 0x18 - resets firmware time-sync state machine
    for (std::size_t i = 3; i < TimeReportSize; ++i) {
        REQUIRE(pkt[i] == 0x00);
    }
}
...
TEST_CASE("ak980 setTime preamble: report id 0x00 + data 0x04 + opcode 0x28 + marker (65-byte)",
          "[proprietary][protocol][clock]") {
    auto const pkt = buildSetTimePreamble();
    REQUIRE(pkt.size() == TimeReportSize);
    REQUIRE(pkt[0] == 0x00);
    REQUIRE(pkt[1] == ReportId);   // 0x04 - first data byte
    REQUIRE(pkt[2] == CmdSetTime); // 0x28
    REQUIRE(pkt[9] == 0x01);       // configure-mode marker
    // All other bytes must be zero.
    for (std::size_t i = 0; i < TimeReportSize; ++i) {
        if (i == 1 || i == 2 || i == 9) {
            continue;
        }
        REQUIRE(pkt[i] == 0x00);
...
TEST_CASE("ak980 setTime data packet: report id 0x00 + magic 0x5A + delimiter 0xAA 0x55 (65-byte)",
          "[proprietary][protocol][clock]") {
    ...
    REQUIRE(pkt[0] == 0x00); // HID report id (unnumbered)
    REQUIRE(pkt[1] == 0x00);
    REQUIRE(pkt[2] == 0x01); // LCD-select index + 1
    REQUIRE(pkt[3] == 0x5a); // magic
    REQUIRE(pkt[4] == 26);   // year-2000
    REQUIRE(pkt[5] == 5);    // month
    REQUIRE(pkt[6] == 17);   // day
    REQUIRE(pkt[7] == 14);   // hour
    REQUIRE(pkt[8] == 23);   // minute
    REQUIRE(pkt[9] == 45);   // second
    REQUIRE(pkt[10] == 0x00);
    REQUIRE(pkt[11] == 0x00); // wDayOfWeek (Sunday)
    // Bytes 12..62 must be zero padding.
    for (std::size_t i = 12; i <= 62; ++i) {
        REQUIRE(pkt[i] == 0x00);
    }
    REQUIRE(pkt[63] == 0xaa); // delimiter high
    REQUIRE(pkt[64] == 0x55); // delimiter low
}
...
TEST_CASE("ak980 setTime save packet: report id 0x00 + data 0x04 + opcode 0x02 (65-byte)",
    ...
    REQUIRE(pkt[0] == 0x00);
    REQUIRE(pkt[1] == ReportId);   // 0x04 - first data byte
    REQUIRE(pkt[2] == CmdSaveRtc); // 0x02 - distinct from CmdCommitEeprom=0x0E
    // All other bytes must be zero.
    for (std::size_t i = 3; i < TimeReportSize; ++i) {
        REQUIRE(pkt[i] == 0x00);
    }
...
TEST_CASE("ak980 TFT chunk-index encoder - round-trip for canonical values",
    ...
    // Encoder returns {byte1, byte2, byte3}: byte1 = 0x80 marker | high 7 bits,
    // byte2 = low 8 bits, byte3 = middle 8 bits (FUN_004231c0:284,287).
    auto decode = [](std::array<std::uint8_t, 3> const& enc) -> std::uint32_t {
        return static_cast<std::uint32_t>(enc[1]) | (static_cast<std::uint32_t>(enc[2]) << 8) |
               ((static_cast<std::uint32_t>(enc[0]) & 0x7fu) << 16);
    };
    // 0 - first chunk of first frame.
    auto e0 = encodeTftChunkIndex(0);
    REQUIRE(e0[0] == 0x80); // byte 1: marker set, high 7 bits = 0
    REQUIRE(e0[1] == 0x00); // byte 2: low 8 bits
    REQUIRE(e0[2] == 0x00); // byte 3: middle 8 bits
    REQUIRE(decode(e0) == 0);
    ...
    REQUIRE((e2314[0] & 0x80) == 0x80); // marker always set on byte 1
    ...
    // low; the MSB of byte 1 is the chunk marker, NOT part of the index).
    ...
    REQUIRE(eMaxMax[0] == 0xff); // byte 1: 0x80 marker | 0x7F upper-7 bits
}
...
TEST_CASE("ak980 TFT chunk-index encoder - values above 23-bit max saturate",
    ...
    // The encoder silently truncates the high bit: byte 1 = 0x80 | (0xFF & 0x7F)
    // = 0x80 | 0x7F = 0xFF - same as encoding 0x7FFFFF.
    REQUIRE(enc24bit[0] == 0xff);
...
TEST_CASE("ak980 TFT chunk-index encoder always sets the 0x80 marker",
          "[proprietary][protocol][tft]") {
    // The marker bit distinguishes a chunk packet (byte 1 has 0x80 set) from
    // the header packet (byte 1 = 0x7F opcode) per ak980pro_tft_protocol.md §3.
    // Critical invariant: every chunk encoding must have byte 1 (output[0]) with
    // 0x80 set.
    for (std::uint32_t idx : {0u, 1u, 100u, 2314u, 65535u, 324099u, 0xffffffu}) {
        auto const enc = encodeTftChunkIndex(idx);
        REQUIRE((enc[0] & 0x80) == 0x80);
    }
}
...
TEST_CASE("ak980 TFT bulk-begin packet - opcode 0x72 + LCD-select + total chunks",
    ...
    // Feature-report layout per FUN_00422920 + FUN_0044eed0 report-id prepend:
    //   [0]=00 report id  [1]=04 frame  [2]=72 opcode  [3]=lcd+1  [9]/[10]=count.
    auto const pkt = buildScreenBulkBegin(/*lcdSelect=*/0, /*total4kChunks=*/16);
    REQUIRE(pkt.size() == ReportSize);
    REQUIRE(pkt[0] == 0x00);               // HID report id
    REQUIRE(pkt[1] == ReportId);           // 0x04 frame byte
    REQUIRE(pkt[2] == CmdScreenBulkBegin); // 0x72
    REQUIRE(pkt[3] == 0x01);               // lcdSelect 0 -> wire 1
    REQUIRE(pkt[9] == 0x10);               // total_4k_chunks low (16) at byte 9
    REQUIRE(pkt[10] == 0x00);              // total_4k_chunks high at byte 10
